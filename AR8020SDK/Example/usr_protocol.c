...
uint8_t get_grd_status_info(unsigned char *buf, int *len)
{
    ...
    HAL_RET_T ret;
    STRU_WIRELESS_INFO_DISPLAY *pst_bbInfoAddr=(STRU_WIRELESS_INFO_DISPLAY *)&pstart;;
    ...
    pstart = buf;
    *pstart++ = 0;//get_link_status();
    *pstart++ = 0;//get_video_state();
...
uint8_t cmd_ack_handle(void *msg, uint8_t port_id)
{
    ...
    ret= HAL_USB_SendCtrl(buf,TO4TIMES(len+10),port_id);
    if(ret != HAL_OK)
    {
...
uint8_t cmd_usb_bypass_read_handle(void *msg, uint8_t port_id)
{
    ...
    ret= HAL_USB_SendCtrl(bypass_buf,TO4TIMES(len+10),port_id);
    if(ret != HAL_OK)
    {
        DLOG_Error("failed %x",ret);
        return 0;
...
uint8_t cmd_usb_bypass_read_uart5_handle(void *msg, uint8_t port_id)
{
    ...
    ret= HAL_USB_SendCtrl(bypass_uart_buf,TO4TIMES(len+10),port_id);
    if(ret != HAL_OK)
    {
